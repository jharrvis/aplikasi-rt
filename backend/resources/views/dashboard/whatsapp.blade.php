...
<div class="bg-white rounded-lg shadow-md p-6 mb-6">
    <div class="flex justify-between items-center mb-4">
        ...
        <span id="wa-connection-badge" class="px-2 py-1 rounded text-sm font-bold bg-gray-200 text-gray-600">Loading...</span>
    </div>
    ...
    <div id="wa-content" class="flex flex-col items-center p-8 border-2 border-dashed border-gray-200 rounded-lg">
        ...
        <div id="wa-qr-container" class="hidden mb-6 p-4 border rounded bg-white shadow-sm">
            ...
            <p class="text-xs text-center text-red-500 mt-1 hidden" id="qr-error-msg">Gagal memuat QR Code. Pastikan service berjalan.</p>
        </div>

        <!-- Actions -->
        <div class="flex gap-4">
            <button id="wa-logout-btn" class="hidden bg-red-500 hover:bg-red-600 text-white font-bold py-2 px-6 rounded-lg transition shadow-md">
                Disconnect / Logout
            </button>
             <button id="wa-reload-btn" class="bg-blue-500 hover:bg-blue-600 text-white font-bold py-2 px-6 rounded-lg transition shadow-md">
            ...
            try {
                ...
                if (data.qr) {
                    ...
                    qrImage.onerror = () => {
                        ...
                        qrErrorMsg.textContent = 'Gagal memuat gambar QR.';
                        qrErrorMsg.classList.remove('hidden');
                    };
                } else if (data.error) {
                    qrContainer.classList.remove('hidden');
                    qrImage.classList.add('hidden'); // Hide broken image if error
                    qrErrorMsg.textContent = data.error;
                    qrErrorMsg.classList.remove('hidden');
                }
            } catch (error) {
                ...
                qrErrorMsg.textContent = 'Network error interacting with backend.';
                qrErrorMsg.classList.remove('hidden');
            }
        }
